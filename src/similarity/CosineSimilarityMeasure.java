...
public class CosineSimilarityMeasure {
	...
	public double get_cosine_similarity_score(boolean granularized)
	{
		...
		try
		{
			...
			{
				int val=Integer.parseInt(map1.get(hset1.toArray()[i])!=null?map1.get(hset1.toArray()[i]).toString():"0");
				sqr1+=val*val;
			}
			...
			{
				int val=Integer.parseInt(map2.get(hset2.toArray()[i])!=null?map2.get(hset2.toArray()[i]).toString():"0");
				sqr2+=val*val;
			}
			...
			for (int i = 0; i < hset1.size(); i++) {
				...
				double val2=Double.parseDouble(map2.get(key)!=null?map2.get(key).toString():"0");
				top_part+=val1*val2;
			}
